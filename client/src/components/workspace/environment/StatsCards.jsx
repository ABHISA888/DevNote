      ...
      {stats.map((stat) => (
        <div 
          ...
          className={`flex flex-col justify-center rounded-xl border border-gray-200 p-5 shadow-sm transition hover:shadow-md ${
            stat.isSecure ? 'bg-primary-50/50 border-primary-100' : 'bg-white'
          }`}
        >
          <div className="flex items-center gap-2 mb-2">
            <span className={`text-[10px] font-extrabold uppercase tracking-widest ${
              stat.isSecure ? 'text-primary-600' : 'text-slate-500'
            }`}>
              {stat.label}
            </span>
          </div>
          ...
          <div className="flex items-baseline gap-2">
            <span className={`text-2xl font-black tracking-tight ${
              stat.isSecure ? 'text-primary-900' : 'text-slate-800'
            }`}>
              {stat.value}
            </span>
            {stat.subtext && (
              <span className="text-[10px] font-bold text-primary-600">{stat.subtext}</span>
            )}
            {stat.isSecure && (
              <Lock size={14} className="text-primary-600 ml-1" />
            )}
          </div>
        </div>
      ))}
